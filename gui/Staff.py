...
class StaffView(ctk.CTkTabview):
    ...
    def on_press(self, data):
        ...
        print(self.updated_staff)
            
    def view_staff(self):
        self.tab("View Staff").columnconfigure((1, 2), minsize=1000)
        self.tab("View Staff").rowconfigure(2, minsize=245)

        self.label = ctk.CTkLabel(master=self.tab("View Staff"), text="View all Branch Staff",
                                  font=self.font)
        self.label.grid(row=0, column=0, padx=20, pady=10, sticky="w")
        
        self.scrollable_frame = ctk.CTkScrollableFrame(master=self.tab("View Staff"),
    ...
    def create_staff(self):
        ...
        self.tab("Create Staff").columnconfigure((0, 1, 2), minsize=1000)
        self.tab("Create Staff").rowconfigure(5, minsize=385)

        self.label = ctk.CTkLabel(master=self.tab("Create Staff"), text="Create a staff member",
                                  font=self.font)
        self.label.grid(row=0, column=0, padx=20, pady=10, sticky="w")

        self.full_name_label = ctk.CTkLabel(master=self.tab("Create Staff"), text='Full Name')
        self.full_name_label.grid(row=1, column=0, padx=20, pady=5, sticky="w")

        self.full_name_entry = ctk.CTkEntry(master=self.tab("Create Staff"), textvariable=
                                       self.full_name, width=350)
        self.full_name_entry.grid(row=1, column=0, padx=(150,0), pady=5, sticky="w")

        self.username_label = ctk.CTkLabel(master=self.tab("Create Staff"), text='Username')
        self.username_label.grid(row=2, column=0, padx=20, pady=5, sticky="w")

        self.username_entry = ctk.CTkEntry(master=self.tab("Create Staff"), textvariable=
                                       self.username, width=350)
        self.username_entry.grid(row=2, column=0, padx=(150,0), pady=5, sticky="w")

        self.password_label = ctk.CTkLabel(master=self.tab("Create Staff"), text='Password')
        self.password_label.grid(row=3, column=0, padx=20, pady=(30,10), sticky="nw")

        self.password_entry = ctk.CTkEntry(master=self.tab("Create Staff"), textvariable=
                                       self.password, width=350)
        self.password_entry.grid(row=3, column=0, padx=(150,0), pady=5, sticky="w")

        self.error_msg = ctk.CTkLabel(master=self.tab("Create Staff"), text="")
        self.error_msg.grid(row=5, column=0, padx=(150,0), pady=(30,10), sticky="nw")
        
        combobox_var = ctk.StringVar(value="Select Role")
        self.drop = ctk.CTkComboBox(master=self.tab("Create Staff"), values=self.dropdown,
                                    ...
                                    text_color='black')
        self.drop.grid(row=4, column=0, padx=(150,0), pady=25, sticky="nw")

        self.create_msg = ctk.CTkLabel(master=self.tab("Create Staff"), text="")
        ...
        self.create_frame.grid(row=6, column=0, sticky="sew", pady=(0,10), padx=10)
    ...
    def update_staff(self):
        ...
        self.tab("Update Staff").columnconfigure((0, 1, 2), minsize=1000)
        self.tab("Update Staff").rowconfigure(5, minsize=450)

        self.label = ctk.CTkLabel(master=self.tab("Update Staff"), text="Update Staff Member",
                                  font=self.font)
        self.label.grid(row=0, column=0, padx=20, pady=10, sticky="w")

        self.update_label = ctk.CTkLabel(master=self.tab("Update Staff"),
                                      text="Please select a branch first")
        self.update_label.grid(row=1, column=0, padx=20, pady=5, sticky="w")
        
        self.new_fullname_label = ctk.CTkLabel(master=self.tab("Update Staff"), text="New Full Name")
        self.new_fullname_label.grid(row=2, column=0, padx=20, pady=5, sticky="w")

        self.new_fullname_entry = ctk.CTkEntry(master=self.tab("Update Staff"), textvariable=
                                       self.new_full_name, width=350)
        self.new_fullname_entry.grid(row=2, column=0, padx=(160,0), pady=5, sticky="nw")

        self.new_role_label = ctk.CTkLabel(master=self.tab("Update Staff"), text="New Role")
        self.new_role_label.grid(row=3, column=0, padx=20, pady=5, sticky="nw")

        self.new_role = None
        combobox_var = ctk.StringVar(value="Select Role")
        self.update_drop = ctk.CTkComboBox(master=self.tab("Update Staff"), values=self.dropdown,
                                    variable=combobox_var, command=self.update_combobox_callback,
                                    width=200, height=35, font=self.drop_font,
                                    fg_color="#f2f2f2", bg_color="#333333",
                                    text_color='black')
        self.update_drop.grid(row=3, column=0, padx=(160,0), pady=25, sticky="nw")

        self.update_msg = ctk.CTkLabel(master=self.tab("Update Staff"), text="")
        self.update_msg.grid(row=4, column=0, padx=(160,0), pady=5, sticky="nw")

        self.update_frame = ctk.CTkFrame(master=self.tab("Update Staff"), fg_color="#333333")
        self.update_frame.grid(row=5, column=0, sticky="sew", pady=(0,10), padx=10)

        self.staff_update_btn = ctk.CTkButton(master=self.update_frame,
                                      text='Update Staff', command=lambda:
                                      self.update_record(self.updated_staff))
        self.staff_update_btn.grid(padx=10, row=0, column=0)
        ...
        self.update_error_msg = ctk.CTkLabel(master=self.tab("Update Staff"), text="")
